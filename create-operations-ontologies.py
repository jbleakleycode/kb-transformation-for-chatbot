...
def openai_call(prompt):
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are an assistant helping to identify labels for the document which will be applied to an ontology. If the response to the question is yes, please response in the format 'yes' and 'no' if otherwise. Response must be lowercase and should not include comment or punctuation. You may be asked multiple questions. Please give yes or no answers in the same order as the questions."},
